...
Each test independently creates an operating-system temporary directory, a SQLite database, random application key and administrator credentials. By default it invokes the production `scripts/admin.mjs init` command, then starts the already-built Next.js application on an available local port. First-run tests opt out of CLI initialization and create the administrator through the real setup page. Tests never reuse a running development server or a developer's database. Fixture teardown terminates only its own child process, verifies the temporary directory boundary and removes the database and related files. Do not run another web build while these tests are running.
...
Page-state regressions verify real polling, failed retries, recovery and distinct time-range caches. A transient RPC outage preserves the last successful value and timestamp; a real logout followed by an authentication denial hides cached administrator data. Mobile checks cover alertdialog cancellation/focus restoration, real API Key revocation, the last-administrator role/disable guards, discarded form input and zero-quota display. Activation defaults to password-only, with explicit opt-in to a real virtual Passkey; unsupported-browser and short-password paths remain covered.
